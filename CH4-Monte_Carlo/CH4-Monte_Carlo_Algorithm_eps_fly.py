...
class Environment():
    def __init__(self):
        ...
        # add s13 to s1, and s9 to s6(fly)
        self.action_to_number = {"up": 0, "right": 1,
                                 "down": 2, "left": 3, "fly": 4}
        self.action_dict = {
            "up": [-1, 0], "right": [0, 1], "down": [1, 0], "left": [0, -1], "fly": [0, 0]}
        self.direction_dict = {0: "up", 1: "right",
                               2: "down", 3: "left", 4: "fly"}
    ...
    def transfer_state(self, state_coordinates, action):
        ...
        if action == "fly":
            if (current_state_coordinates == np.array([2, 4])).all():
                next_state_coordinates = np.array([0, 1])
                return next_state_coordinates
            elif (current_state_coordinates == np.array([1, 3])).all():
                next_state_coordinates = np.array([1, 0])
                return next_state_coordinates
        # -----------------------------
...
class Monte_Carlo():
    ...
    def policy(self, state_coordinate):  # Optimal policy, find the maximun Q(s,a) and return action
        ...
        if self.flag == 1:
            ...
            # if there are not only one max
            return self.env.direction_dict[random.choice(indexes)]
            # ------------------------------------
        elif self.flag == 0:  # prevent random when print
            for valid_action in valid_actions:
                if max_value == self.Q_values[(tuple(state_coordinate), valid_action)]:
                    indexes.append(self.env.action_to_number[valid_action])
            return self.env.direction_dict[random.choice(indexes)]
